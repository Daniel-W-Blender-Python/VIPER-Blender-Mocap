...
The addon extracts 3D joint locations from the image, using the Metrabs model, and marks those locations with empties in the 3D space. The addon then constrains the locations of the hands and feet to the corresponding empties.

Since the Metrabs model only extracts static poses from images, the addon extracts the frames of the video into the "Video Frames" folder of the addon.

The addon can extract the motion of multiple people, as well as their respective positions in the 3D space.
